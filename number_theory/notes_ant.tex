...
\begin{defbox}
    \begin{definition}[Trace and Norm]
        ...
        \end{equation*}
        We define trace of \(\alpha\) by \(\mathrm{Tr}_K(\alpha) := \mathrm{Tr}(\Phi_\alpha)\) and the norm of \(\alpha\) by \(N(\alpha) := \det(\Phi_\alpha)\).
    \end{definition}
\end{defbox}

\begin{example}
    In this example, we will compute the traces and norms of some concrete algebraic number fields.
    \begin{enumerate}
    \item Let \(K = \mathbb{Q}(i)\). If \(\alpha = a + ib\) with \(a, b \in \mathbb{Q}\), then \(\mathrm{Tr}_K(\alpha) = 2a\) and \(\mathrm{N}_K(\alpha) = a^2 + b^2\).
    \begin{proof}
        ...
        \begin{align*}
            ...
            \end{pmatrix} \text{.}
        \end{align*}
        Therefore, \(\mathrm{Tr}_K(\alpha) = 2a\) and \(\mathrm{N}_K(\alpha) = a^2 + b^2\).
    \end{proof}
    \item Let \(K = \mathbb{Q}(\sqrt{2})\). If \(\alpha = a + \sqrt{2}b\) with \(a, b \in \mathbb{Q}\), then \(\mathrm{Tr}_K(\alpha) = 2a\) and \(\mathrm{N}_K(\alpha) = a^2 - 2b^2\).
    \begin{proof}
        ...
        \begin{align*}
            ...
            \end{pmatrix} \text{.}
        \end{align*}
        So we have \(\mathrm{Tr}_K(\alpha) = 2a\) and \(\mathrm{N}_K(\alpha) = a^2 - 2b^2\).
    \end{proof}
...
\begin{example}
    In this example, we will look at the cases when the field extension is given by a root of a polynomial \footnote{The field extensions are defined by a single root of a polynomial, but they are not splitting fields (I think).}.
    \begin{enumerate}
        \item Let \(K = \mathbb{Q}(\theta)\) where \(\theta\) is a root of \(X^3 - X - 1\).
        \begin{proof}
            A basis of \(K\) is \(\{1, \theta, \theta^2\}\). If \(\theta^2\) is lineary dependent of \(1\) and \(\theta\), then 
        \end{proof}
    \end{enumerate}
\end{example}
