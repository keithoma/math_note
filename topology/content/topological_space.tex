...
\begin{example}
    ...
    \end{align*}
    and has \(8\) elements. There are \(2^8 = 256\) subsets of \(\mathcal{P}(X)\)\footnote{The number of subsets in the power set is \(2^8\) because for each element one can choose to be inside the subset or not.}.
    \begin{enumerate}
        ...
        \item Any collection of three subsets of \(X\) is a well-defined topology as long as it contains the emptyset and the entire set \(X\). Therefore, \(\set{\varnothing, \{1\}, X}\), \(\set{\varnothing, \{3\}, X}\), and \(\set{\varnothing, \{2, 3\}, X}\) are all examples of a topology. In this case, \(\{2, 3\}\), \(\{1, 2\}\), and \(\{1\}\) are closed subsets respectively. \(\varnothing\) and \(X\) are the only clopen subsets for each of the given topologies. In fact, for any topology that can be defined on \(X\), the only clopen subsets are the emptyset and \(X\) itself. However, \(\set{\{1\}, \{2\}, X}\), \(\set{\varnothing, \{1\}, \{2, 3\}}\), \(\set{\{1\}, \{2\}, \{3\}}\) are all nonexamples of a topology.
        \item Since any topology must include the union of members, the only topologies on \(X\) with four elements contains a subset that is included in the other one that is not \(X\). For example, \(\set{\varnothing, \{1\}, \{1, 3\}, X}\) and \(\set{\varnothing, \{2\}, \{2, 3\}, X}\) are topologies, but \(\set{\varnothing, \{1\}, \{2, 3\}}\) and \(\set{\varnothing, \{1\}, \{2\}, X}\) are not. In this example, \(\set{\varnothing, \{2\}, \{2, 3\}, X}\) and \(\set{\varnothing, \{1\}, \{1, 3\}, X}\) are the collection of closed subsets respectively.
        ...
        \item Lastly, \(\mathcal{P}(X)\) itself is a well-defined topology.
    \end{enumerate}
...
\begin{rembox}
    \begin{remark}
        Openness and closedness of a subset does not imply anything of the other property. An open subset may also be closed and vice versa. Furthermore, a subset need not be open nor closed.
    \end{remark}
\end{rembox}
%
\begin{defbox}
