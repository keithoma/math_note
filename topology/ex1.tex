...
\begin{lemma}
    The preimage of a map is stable under finite intersections, i.e. let \(f: X \longrightarrow Y\) be a map, and \(A\) and \(B\) subset of \(Y\), then it is
    \begin{align*}
        f^{-1} \left(A \cap B\right) = f^{-1}\left(A\right) \cap f^{-1}\left(B\right) \text{.}
    \end{align*}
\end{lemma}
\begin{proof}
    We simply have
