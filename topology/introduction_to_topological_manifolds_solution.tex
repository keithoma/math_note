...
If \(h\) is constant, say \(h(Y) = \{p\}\), then \(h^{-1}(U) = Y\) if \(p \in U\) and \(h^{-1}(U) = \varnothing\) if \(p \in U\). In both cases the preimages are open, thus \(h\) is continuous. Assume \(h\) is continuous but not constant.
