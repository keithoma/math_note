...
\begin{enumerate}
    \item Assume \(U\) is open in \(X_2\). Because \(f\) is continuous, the image of open subsets are again open, thus \(f(U)\) lies in \(\mathcal{T}_2\).
...
``\(\Leftarrow\)'': Let \(f: X_1 \longrightarrow X_2\) be a bijective map such that \(f(\mathcal{T}_1) = \mathcal{T}_2\). Fix a open subset \(V \in \mathcal{T}_2\).
\end{document}